...
metropolis<-function(x1,x2,sigmastar1=0.2,sigmastar2=0.1){
  ...
  #y1 and y2
  if (runif(1) > (target(y1, y2) / target(x1, x2)) *
        (dnorm(x1, y1, sigmastar1) /
           dnorm(y1, x1, sigmastar1)) *
        (dtruncnorm(x2, a = 0, b = Inf, mean = y2, sd = sigmastar2) /
           dtruncnorm(y2, a = 0, b = Inf, mean = x2, sd = sigmastar2))) {
    y1 = x1
    y2 = x2
  }
  #return y
...
# run the algorithm N times (irrelevant of
# acceptance/rejection of candidate point)
N<-10000
#create chain x
...
plot(chain[,2],type='l',ylab="sigma",main="Trace Plot of sigma")

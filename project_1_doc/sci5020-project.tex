...
$$
L(\alpha, x_m; x) = \prod_{i=1}^{n} \frac{\alpha x_{m}^{\alpha}}{x_i^{\alpha+1}}
$$

\noindent then,

\begin{flalign*}
l(\alpha, x_m;x) &= \sum_{i=1}^{n} \log \left(  \frac{\alpha x_{m}^{\alpha}}{x_i^{\alpha+1}} \right)\\
...
%
%$$
%\frac{\partial l(\alpha, x_m; x)}{\partial x_m} = \frac{n \alpha}{x_m} \quad x_m \le x_i
%$$

\noindent As the probability density function is only defined for $x_i \ge x_m$, the likelihood is zero if $x_m > \min(x_1 \dots x_n)$.
...
$$
\frac{\partial l(x; \alpha, x_m)}{\partial x_m} = \frac{\alpha}{x_m} ;\quad x\ge x_m
$$

Taking the second derivative with respect to $x_m$
$$
\frac{\partial^2 l(x; \alpha, x_m)}{\partial x_m^2} = -\frac{\alpha}{x_m^2} ;\quad x\ge x_m
$$

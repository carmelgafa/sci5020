...
Q_prev <- -Inf
Q_new <- sum.finite(log(pi1)+log(dnorm(x, mu1, sigma1))) + sum.finite(log(pi2)+log(dnorm(x, mu2, sigma2)))
k<-2
while (abs(Q_new-Q_prev)>=1e-6) {
  #E-step
  ...
  p1 <- pi1
  p2 <- pi2
  print(c(mu1,mu2,sigma1,sigma2,p1,p2))
  k <- k + 1
  Q_new <- sum.finite(log(pi1)+log(dnorm(x, mu1, sigma1))) + sum.finite(log(pi2)+log(dnorm(x, mu2, sigma2)))

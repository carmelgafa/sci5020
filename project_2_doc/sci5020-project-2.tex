...
The algorithm is initialized with an arbitrary value for $\tau^{(0)}$. At each iteration $t$, new values for $\mu$ and $\tau$ are drawn from their respective full conditionals using the most recent values of the other parameter. This process is repeated for a total of 10,000 iterations, with an appropriate number of samples discarded as burn-in.
\textbf{}
